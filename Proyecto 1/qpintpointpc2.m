function [x, l, mu, z, iter, fvals] = qpintpointpc2(Q, A, F, b, c, d)
    % Version vieja, ignorar
    %Params
    MAX_ITER = 100;
    TOL = 1e-05;
    iter = 0;
    ...
    while norm([rx;rl;rmu;rz]) > TOL && iter < MAX_ITER
        ...
        delta_mu = ones(p, 1) - (rmu.*delta_z)./z;
        ...
        gamma = dot(z + alpha_k*delta_z, mu + alpha_k*delta_mu)/p;
        gamma = (gamma/gamma_size)^3;
        gamma = gamma_size * gamma;

        % Modificar r para el sistema correctivo
        r(1:n) = r(1:n) - F'*(((alpha_k^2)*delta_z.*delta_mu + gamma)./z);
